...
class NeonListView extends StatefulWidget {
  ...
  /// Creates a Neon list view with multiple custom slivers.
  NeonListView.children({
    required this.isLoading,
    required this.error,
    required this.onRefresh,
...
class _NeonListViewState extends State<NeonListView> {
  // we need a stateful widget because otherwise we get at the end of loading one more refresh
  // of the entire widget which we want to avoid
  ...
  Widget build(BuildContext context) {
    ...
    final children = widget.slivers.map(
