...
class BlurBloc extends Bloc {
  ...
  final _blurHashCash = <String, BlurHashDecodeTask>{};
  ...
  Future<ui.Image>? getBlurHash(String? blurHash, ui.Size size, {bool cache = true}) {
    ...
    if (_blurHashCash[task.key] != null) {
      return _blurHashCash[task.key]!.result.future;
    }
    ...
    if(cache) {
      _blurHashCash[task.key] = task;
    }
    return task.result.future;
  }

  /// Removes the cached image for the given [blurHash] and [size].
